...
def create_db_record(db_dict, ingest_endpoint):
    ...
    try:
        ...
        print(f"Log {dynamodb_table} put response: {res}")
    except Exception as e:
        raise e
...
def call_sfn(sfn_arn, payload):
    try:
        ...
        print(f"Successfully invoked sfn {sfn_arn}")
        print(response)
    except Exception as e:
        print(str(e))
        print(f"Error: sfn={sfn_arn} payload={payload}")
    return
...
def lambda_handler(event, context):
    ...
    ingest_endpoint = v.get_feed_ingest_endpoint(season_category_id,
                                                 publisher_id)  # get ingest endpoint for that season
